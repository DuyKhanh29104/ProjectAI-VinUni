...
@pytest.mark.asyncio
async def test_inference_token_and_object_scope_are_enforced_before_storage(monkeypatch):
    def unexpected_storage(*args, **kwargs):
        pytest.fail("Rejected requests must not access GCS")

    monkeypatch.setattr("src.inference_app.create_gcs_storage_client", unexpected_storage)
    application = create_inference_app(
        settings=InferenceServiceSettings(_env_file=None, inference_auth_token="test-only-token"),
        gcs_settings=IngestionSettings(_env_file=None, gcs_bucket="bucket"),
    )
    request = {
        "image": {
            ...
            "split": "product",
            "imageId": "1",
            "objectKey": "outside/image.jpg",
            "bucket": "bucket",
        }
    }
    async with AsyncClient(transport=ASGITransport(app=application), base_url="http://test") as client:
        assert (await client.post("/v1/detect", json=request)).status_code == 401
        response = await client.post(
            "/v1/detect",
            json=request,
            headers={
                "X-Label-Guardian-Inference-Token": "test-only-token",
            },
        )
        assert response.status_code == 403


def test_production_inference_requires_token():
    from pydantic import ValidationError

    with pytest.raises(ValidationError, match="INFERENCE_AUTH_TOKEN"):
        InferenceServiceSettings(_env_file=None, inference_app_env="production", inference_auth_token=None)
