...
**Nguyên nhân:** Chưa cài Python 3.12 hoặc không có trong PATH.
...
```bash
python -m pip install -e ".[agent,ingestion]" --group dev -i https://pypi.tuna.tsinghua.edu.cn/simple
# Hoặc
python -m pip install -e ".[agent,ingestion]" --group dev -i https://mirror.cloudflare.com/pypi/simple
```
...
**Nguyên nhân:** Docker không cache layer do `pyproject.toml` thay đổi, hoặc network issue.
...
2. Đảm bảo `Dockerfile` và `pyproject.toml` có trong repo
3. Thêm `runtime.txt` với nội dung `3.12.x` nếu Render chọn sai Python version
...
5. **Xóa và tạo lại** — tạo venv mới, sau đó chạy `python -m pip install -e ".[agent,ingestion]" --group dev`
