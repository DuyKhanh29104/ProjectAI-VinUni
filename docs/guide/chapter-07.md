...
**Layer caching:** Docker build theo từng layer (từng lệnh trong Dockerfile). Layer cài package chỉ bị invalidated khi `pyproject.toml`, `README.md` hoặc source package thay đổi; thay đổi ở các file được copy sau đó không buộc Docker cài dependencies lại.
  ...
  lint:
    ...
    steps:
      ...
      - name: Install quality tools
        run: python -m pip install --group quality
...
**Cache pip dependencies:** Action `actions/cache` lưu cache của pip, tránh download lại 100+ packages mỗi lần chạy. Key cache dựa trên hash của `pyproject.toml` — chỉ invalidate khi dependencies thay đổi.
