...
- **Python 3.12 hoặc mới hơn.** Dùng Python 3.12.x để khớp với metadata package, CI và Docker của dự án.
...
```bash
# Cài application, optional runtimes và development dependencies
$ python -m pip install -e ".[agent,ingestion]" --group dev

# Hoặc nếu lệnh trên không hoạt động, cài từng bước:
$ python -m pip install -e ".[agent,ingestion]"
$ python -m pip install --group dev
```

Flag `-e` (editable) có nghĩa là bạn cài package ở chế độ "có thể chỉnh sửa" — khi bạn sửa code trong `src/`, thay đổi sẽ phản ánh ngay lập tức mà không cần cài lại. Các extra `agent`/`ingestion` bật runtime tùy chọn; dependency group `dev` cài công cụ testing, linting và type checking.
