...
class RemoteInferenceClient:
    ...
    async def _post(self, endpoint: str, request: InferenceRequest | InferenceBatchRequest) -> object:
        ...
        try:
            ...
            raise InferenceClientError("Inference service request failed.") from error

        if response.status_code >= 400:
            raise InferenceClientError(f"Inference service returned HTTP {response.status_code}.")
