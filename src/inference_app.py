...
def _run_yolo_batch(
    ...
    *,
    ...
    item_results = [(_detections_from_result(result), _result_latency(result)) for result in results]
    latency_ms: dict[str, float | None] = {
        "model_load": round(model_load_ms, 3),
...
def create_inference_app(
    *,
    ...
    service_settings = settings or InferenceServiceSettings()
    storage_settings = gcs_settings or IngestionSettings()
    ...
    def detect_batch(request: InferenceBatchRequest) -> InferenceBatchResponse:
        if len(request.images) > service_settings.inference_max_batch_size:
            raise HTTPException(
                status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
                detail=(f"Batch size exceeds inference_max_batch_size={service_settings.inference_max_batch_size}."),
            )
        ...
        image_payloads = [image_bytes for (_image, _bucket, _object_key, _content_type, image_bytes) in payloads]
        with _inference_lock:
        ...
        batch_latency_ms: dict[str, float | None] = {
            **latency_ms,
