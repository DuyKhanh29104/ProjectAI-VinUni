...
test("application sidebar expands on hover and omits the API status card", () => {
  ...
  assert.match(
    themeStyles,
    /\.app-shell-dark \.sidebar\s*{[\s\S]*?width: 68px;/,
  );
  assert.match(
    themeStyles,
    /\.app-shell-dark \.sidebar:hover\s*{\s*width: 220px;/,
  );
  assert.match(
    themeStyles,
    /\.app-shell-dark \.sidebar:hover\s*{[^}]*transition-delay: 70ms;/,
  );
  assert.match(
    themeStyles,
  ...
  assert.doesNotMatch(
    layoutSource,
    /API connected · review actions are audited\./,
  );
...
test("all application scrollbars use semantic surface tokens", () => {
  ...
  assert.match(
    baseStyles,
    /scrollbar-color: var\(--color-surface-3\) var\(--color-canvas\);/,
  );
  assert.match(
    baseStyles,
...
test("QA Cases omits implementation and reviewer context chips", () => {
  const apiQueueSource = readSource(
    "../src/features/qa-queue/ApiQAQueueView.tsx",
  );
...
test("QA comparison viewers pan by pointer drag instead of scrollbars", () => {
  ...
  assert.match(
    queueStyles,
    /\.queue-viewer-stage\.is-dragging\s*{\s*cursor: grabbing;/,
  );
});
...
test("QA case risk filters use bounded numeric inputs", () => {
  ...
  const mockQueueSource = readSource(
    "../src/features/qa-queue/MockQAQueueView.tsx",
  );
  ...
  for (const source of [apiQueueSource, mockQueueSource]) {
    ...
    assert.doesNotMatch(
      source,
      /className="queue-risk-filter"[\s\S]{0,300}type="range"/,
    );
  }
});
...
test("API dataset filter updates the URL-scoped dataset", () => {
  ...
  assert.match(
    apiQueueSource,
    /setSearchParameters\(\s*\{\s*dataset:\s*event\.target\.value,\s*split:\s*"product",?\s*\}\s*\)/,
  );
