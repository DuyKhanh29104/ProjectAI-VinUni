...
test("QA Queue filters by dataset/status/risk and sorts by priority or risk", () => {
  ...
  const assignedCase = state.findings.find(
    (finding) => finding.id === "finding-006",
  );
  assert.ok(assignedCase);
  assert.equal(
    canEditAnnotation(assignedCase, "reviewer", "user-reviewer"),
    true,
  );
  assert.equal(
    canEditAnnotation(assignedCase, "annotator", "user-annotator"),
    true,
  );
  assert.equal(canEditAnnotation(assignedCase, "admin", "user-admin"), false);
  assert.equal(
    canSubmitAnnotatorFeedback(assignedCase, "annotator", "user-reviewer"),
    false,
  );
...
test("role switch and review actions update mock state", () => {
  ...
  state = repository.setFindingStatus(
    state,
    "finding-001",
    "in_review",
    "user-reviewer",
  ...
  assert.equal(
    state.findings.find((finding) => finding.id === "finding-001")?.status,
    "in_review",
  );
  ...
  assert.equal(
    state.findings.find((finding) => finding.id === "finding-002")?.status,
    "rejected",
  );
...
test("assignment, anchored feedback and rework use workflow stage independently", () => {
  ...
  const requested = state.findings.find(
    (finding) => finding.id === "finding-001",
  );
  ...
  assert.equal(
    state.findings.find((finding) => finding.id === "finding-001")
      ?.workflowStage,
    "resubmitted",
  );
...
test("repository proposal protects original and records approval", () => {
  ...
  const original = state.annotations.find(
    (annotation) => annotation.id === "annotation-001",
  );
  assert.ok(original);
  const originalBox = { ...original.bbox };

  state = repository.saveProposedAnnotation(
    state,
    "annotation-001",
    {
      label: "van",
  ...
  const proposal = state.annotations.find(
    (annotation) => annotation.id === "annotation-001::proposed",
  );
  const unchangedOriginal = state.annotations.find(
    (annotation) => annotation.id === "annotation-001",
  );
  ...
  state = repository.approveFinding(
    state,
    "finding-001",
    "user-reviewer",
    "Approve mock correction",
  );
  const approved = state.annotations.find(
    (annotation) =>
      annotation.layer === "approved" &&
      annotation.sourceAnnotationId === "annotation-001",
  );
...
test("QA run and rule/model configuration stay in frontend mock state", () => {
  const repository = new MockRepository();
  let state = repository.seed();
  state = repository.updateRule(state, "rule-box-iou", {
    enabled: false,
    threshold: 0.65,
  });
  state = repository.updateModel(state, "model-yolo-reference", {
    confidenceThreshold: 0.8,
  });
  assert.equal(
    state.rules.find((rule) => rule.id === "rule-box-iou")?.enabled,
    false,
  );
  assert.equal(
    state.models.find((model) => model.id === "model-yolo-reference")
      ?.confidenceThreshold,
    0.8,
  );
