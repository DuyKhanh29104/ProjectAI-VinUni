...
export function ReviewHistoryPanel({
  ...
  const original = state.annotations.find(
    (annotation) =>
      annotation.id === finding.annotationId && annotation.layer === "original",
  );
  const proposal = state.annotations.find(
    (annotation) =>
      annotation.layer === "proposed" &&
      ((finding.annotationId &&
        annotation.sourceAnnotationId === finding.annotationId) ||
  ...
  return (
    <Card className="review-history-card">
      ...
      <div className="history-version-strip">
        <div>
          <span>Original</span>
          <strong>{original ? `v${original.version}` : "—"}</strong>
          <small>Immutable</small>
        </div>
        <span className="version-arrow">→</span>
        <div>
          <span>Proposed</span>
          <strong>{proposal ? `v${proposal.version}` : "—"}</strong>
          <small>{proposal ? proposal.updatedBy : "Not created"}</small>
        </div>
        <span className="version-arrow">→</span>
        <div>
          <span>Approved</span>
          <strong>{approved ? `v${approved.version}` : "—"}</strong>
          <small>{approved ? approved.updatedBy : "Not approved"}</small>
        </div>
      </div>
      ...
      {decisions.length > 0 ? (
              ...
              <div className="audit-row" key={decision.id}>
                ...
                <div className="audit-copy">
                  <div className="audit-title">
                    ...
                    <StatusBadge status={decision.toStatus} />
                  </div>
                  <p>
                  ...
                  {decision.changeSummary && decision.reason ? (
                    <small className="audit-reason">
                      Ghi chú: {decision.reason}
                    </small>
                  ) : null}
                  <small>
                    {user?.name ?? decision.userId} ·{" "}
                    {new Date(decision.timestamp).toLocaleString("vi-VN")}
                  </small>
                </div>
              </div>
            );
        ...
        <div className="audit-empty">
          Chưa có quyết định review nào. Các thao tác của reviewer sẽ xuất hiện
          ở đây.
        </div>
      )}
    </Card>
